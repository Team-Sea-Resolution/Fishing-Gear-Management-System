...
ocean_nc_file = r'.\dataset\sediment_data_final_uv.nc'
wind_nc_file  = r'.\dataset\wind.nc'
# wind_nc_file  = r'C:\Users\HUFS\Desktop\침적opendrift\바람데이터_3_9.nc'
bathymetry_nc_file = r'.\dataset\bathymetry_xyz.nc'


coastline_file = r".\dataset\2024년 전국해안선.shp"
traffic_file = r".\dataset\교통량_202505192156.xlsx"
...
# --- 침강 속도 범위 설정 ---
debris_properties = {
    'wood':   {'v_range': (0.15, 0.2)},
    'cloth':  {'v_range': (0.08, 0.13)},
    'bottle': {'v_range': (1.0, 1.5)},
    'rope':   {'v_range': (0.15, 0.2)}
}

debris_types = list(debris_properties.keys())


# 시딩 구간 제한
SEEDING_END = datetime(2024, 3, 10)

for i in range(num_events):
    seed_time = simulation_start + i * seeding_interval
    if seed_time > SEEDING_END:
        continue  # 이후에는 시딩하지 않음

    lons, lats = generate_seed_points_from_traffic()
    types = np.random.choice(debris_types, size=len(lons))
    terminal_velocity = np.array([
        np.random.uniform(*debris_properties[t]['v_range']) for t in types
    ])
